package com.sds.runway.exepl.contentProposal;
...
public abstract class ExContentProposalAdapter implements IPropertyChangeListener {
...
//						item.setImage(getImage(proposal));
	 ...
	 * Default value is PROPOSAL_REPLACE.
	 */
	private int proposalAcceptanceStyle = PROPOSAL_REPLACE;
	 ...
	 */
	public ExContentProposalAdapter(Control control,
		...
		setLabelProvider(getContantProposalLabelProvider());
	}
	...
	public void propertyChange(PropertyChangeEvent event) {
		if(event.getProperty().equals(getMetaSearchKeyBinding())) {
			changeKeyBinding();
		}
	}
			...
			public void handleEvent(Event e) {
				...
				case SWT.Modify:
					...
					if (getControl() instanceof Text) {
						String contentText = ((Text) getControl()).getText();
						closeProposalPopup();
						if (!enterPressed
								&& contentText.length() != 0
								&& isKorean(contentText
										.charAt(contentText.length() - 1))) {
							receivedKeyDown = true;
							watchModify = true;
						} else {
							enterPressed = false;
	...
	public void setTriggerKeyStroke(KeyStroke triggerKeyStroke) {
		...
		if (isValid()) {
			if (popup == null) {
				...
				if (proposals.length > 0) {
					if (isDebug()) {
						log.info("POPUP OPENED BY PRECEDING EVENT"); //$NON-NLS-1$
					}
					recordCursorPosition();
	...
	private void proposalAccepted(IContentProposal proposal) {
		//TODO FIXME XXX 컨텐트 프로포셜 선택값이 결정되어 입력되는 부분.
	...
	private IContentProposal[] getProposals() {
		...
		if (isDebug()) {
			log.info(">>> obtaining proposals from provider"); //$NON-NLS-1$
		}
	...
	private void notifyProposalAccepted(IContentProposal proposal) {
		if (isDebug()) {
			log.info("Notify listeners - proposal accepted."); //$NON-NLS-1$
		}
	...
	private void notifyPopupOpened() {
		...
		for (int i = 0; i < listenerArray.length; i++) {
			proposalPopupOpened((IContentProposalListener2)listenerArray[i]);
		}
	}
	...
	private void notifyPopupClosed() {
		...
		final Object[] listenerArray = proposalListeners2.getListeners();
		for (int i = 0; i < listenerArray.length; i++) {
			proposalPopupClosed((IContentProposalListener2)listenerArray[i]);
		}
	}
	...
	protected abstract boolean isKorean(char cValue) ;

	protected abstract ILabelProvider getContantProposalLabelProvider();

	protected abstract IPreferenceStore getPreferenceStore();

	protected abstract void changeKeyBinding() ;

	protected abstract String getMetaSearchKeyBinding();

	protected abstract void proposalPopupOpened(IContentProposalListener2 listener);
	protected abstract void proposalPopupClosed(IContentProposalListener2 listener);
}
