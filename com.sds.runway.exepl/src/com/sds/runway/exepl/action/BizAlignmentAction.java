package com.sds.runway.exepl.action;
...
public class BizAlignmentAction extends SelectionAction {
	 ...
	 *            the alignment ID
	 */
	public BizAlignmentAction(IEditorPart editor, int align) {
	 ...
	 *            the aligment ID.
	 */
	public BizAlignmentAction(IWorkbenchPart part, int align) {
		super(part);
		alignment = align;
		initUI();
	}
	...
	protected Rectangle calculateAlignmentRectangle(Request request) {
		...
		for (Object object : editparts) {
			...
			if (part.getFigure() instanceof HandleBounds) {
				rectangle = ((HandleBounds) part.getFigure()).getHandleBounds();
			} else {
				rectangle = part.getFigure().getBounds();
			}

			if (unionRect == null) {
				unionRect = rectangle;
			} else {
				unionRect = unionRect.getUnion(rectangle);
			}
		}

		IWorkbenchPart workbenchPart = getWorkbenchPart();
		ScrollingGraphicalViewer viewer = (ScrollingGraphicalViewer) workbenchPart.getAdapter(GraphicalViewer.class);
		FigureCanvas figureCanvas = (FigureCanvas) viewer.getControl();
		Point location = figureCanvas.getViewport().getViewLocation();
		unionRect.translate(-location.x, -location.y);

		Rectangle rect = new PrecisionRectangle(unionRect);
		return rect;
	}
