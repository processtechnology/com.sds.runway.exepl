package com.sds.runway.exepl.control;
 ...
 * @since 3.2
 */
public class MultiLanguageContentProposalAdapter {
	...
	class ContentProposalPopup extends PopupDialog {
		...
		ContentProposalPopup(String infoText, IContentProposal[] proposals) {
			...
			if(proposals == null || proposals.length == 0){
				this.proposals = getEmptyProposalArray();
			}else{
				this.proposals=  Arrays.copyOf(proposals, proposals.length);
			}
		...
		private void setProposals(IContentProposal[] newProposals) {
			if (newProposals == null || newProposals.length == 0) {
				...
				this.proposals=  Arrays.copyOf(newProposals, newProposals.length);
			}
		...
		private void selectProposal(int index) {
			Assert
					.isTrue(index >= 0,
							"Proposal index should never be negative");
	 ...
	 */
	protected ListenerList proposalListeners = new ListenerList();
	 ...
	 */
	public MultiLanguageContentProposalAdapter(Control control,
		...
		if (autoActivationCharacters != null) {
			this.autoActivateString = new String(Arrays.copyOf(autoActivationCharacters, autoActivationCharacters.length));
		}
	...
	public MultiLanguageContentProposalAdapter(Control control,
		...
		// The rest of these may be null
		try {
			this.triggerKeyStroke = KeyStroke.getInstance("Ctrl+Space");
	...
	private void addControlListener(Control control) {
		...
		controlListener = new Listener() {
			...
			private void dump(String who, Event e) {
				...
				sb.append(" - e: keyCode=");
				sb.append(e.keyCode);
				sb.append(hex(e.keyCode));
				sb.append("; character=");
				sb.append(e.character);
				sb.append(hex(e.character));
				sb.append("; stateMask=");
				sb.append(e.stateMask);
				sb.append(hex(e.stateMask));
				sb.append("; doit=");
				sb.append(e.doit);
				sb.append("; detail=");
				sb.append(e.detail);
				sb.append(hex(e.detail));
				sb.append("; widget=");
				sb.append(e.widget);
	 ...
	 */
	protected void notifyProposalAccepted(IContentProposal proposal) {
		if (DEBUG) {
	...
	private boolean isKorean(char cValue){
		if (cValue >= 0x3131 && cValue <= 0xD743) {
			return true;
		}
		return false;
	}
